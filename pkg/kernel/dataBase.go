...
func (client *Client) AddHash(msg, date, login string) error {
	...
	err := json.Unmarshal(Base64Decode(msg), &passLogin)
	if err != nil {
		return err
	}
	sum, err := HashSumAuth(passLogin)
	if err != nil {
		return err
	}
	client.dbExternals.mtx.Lock()
	defer client.dbExternals.mtx.Unlock()
	_, err = client.dbExternals.ptr.Exec(`INSERT OR REPLACE INTO hashes (login, date, hash) VALUES ($1, $2, $3)`, login, date, sum)
	return err
}
...
func GetUserFromDB(user *User) uint {
	psswd := Base64Encode(HashSum([]byte(user.Password)))
...
func HashSumAuth(passLogin string) (string, error) {
	marshal, err := json.Marshal(passLogin)
	if err != nil {
		return "", err
	}
